...
class Product(Agent):
    ...
    def react(self, message):
        ...
        if 'manager' in message.sender.name:
            ...
            if msg_cont[0] == self.name:
                ...
                #self.call_later(8.0, self.sending_to_parts, msg_cont[1])

                self.sending_to_parts(msg_cont[1])

                #self.sending_to_workers(msg_cont[1])

                #self.write_to_workers(msg_cont[1])
                #self.call_later(8.0, self.write_to_workers, msg_cont[1])
        ...
        if 'part' in message.sender.name and message.performative == 'inform-if':
            ...
            if self.count == len(self.parts):
                ...
                print(f'{name} Начал собирать продукт {self.name} для клиента {message.content}. Счет {score}. Время {busy_old}:{busy_new}')
                self.send_to_calendar2(name, message.content, str(score), str(busy_old), str(busy_new), str(full_time), str(wait_time))
                self.count = 0
    ...
    # Отправляет запрос всем агентам-рабочим, чтобы те вернули свой score
    def sending_to_workers(self, client):
        message = ACLMessage(ACLMessage.REQUEST)
        message.set_protocol(ACLMessage.FIPA_REQUEST_PROTOCOL)
        message.set_performative('query-if')
        message.set_sender(self.aid)
        ...
        for i in self.workers:
            pd_res = 'worker' + str(i['num'])
            message.add_receiver(AID(pd_res))
        pd_cont = self.name + ' ' + str(self.time) + ' ' + client
        message.set_content(pd_cont)
        display_message(self.aid.localname,
                        f'Продукт {self.name} отправил сообщение {message.content} всем рабочим')

        self.send(message)


    # Отправляет определенному рабочему сообщение-запрос, чтобы тот изменил свое доступное время
    def write_to_workers(self, client):
        if self.workers:
            ...
            pd_cont = self.name + ' ' + str(self.time) + ' ' + client
            display_message(self.aid.localname, f'{self.name} ВЫДАЛ ЗАДАНИЕ РАБОЧЕМУ!')
            message.set_content(pd_cont)

            self.send(message)
            # self.call_later(2.0, self.send, message)
    ...
    def send_to_calendar2(self, worker_name, client_name, score, busy_old, busy_new, full_time, wait_time):
        message = ACLMessage(ACLMessage.INFORM)
